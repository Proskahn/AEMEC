#!/usr/bin/env python3
"""Overlay stored polarization curves for selected membrane thicknesses."""
...
def _load_curve(
    ...
    for row in rows:
        try:
            ...
            raise ValueError(f"invalid I-V data in: {curve_path}") from exc
        if not math.isfinite(current) or not math.isfinite(cell_voltage):
            raise ValueError(f"non-finite I-V data in: {curve_path}")
...
def plot_curves(curves: Sequence[TrialCurve], output_path: Path) -> None:
    ...
    markers = ("o", "s", "^", "D", "v", "P", "X")
    figure, axes = plt.subplots(figsize=(7.2, 5.2), constrained_layout=True)
    ...
    axes.set_title("Polarization Curves by Membrane Thickness")
    axes.grid(True, which="major", alpha=0.3)
    axes.ticklabel_format(axis="x", style="plain")
    axes.legend(title="Membrane thickness")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        help="Output PNG path (default: selected_polarization_curves.png in study).",
    )
    return parser
